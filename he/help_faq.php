...
$help = array(
	...
	array(
		...
		1 => 'ישנן כמה סיבות אשר יכולות להיות התשובה לבעיה. ראשית, בדוק ששם המשתמש והסיסמה שהזנת הם אכן נכונים. אם בדקת והם נכונים, צור קשר עם מנהל המערכת וברר אם אינך חסום מגישה למערכת. אפשרות נוספת היא שמנהל המערכת אחראי לבעיה והתקלה היא במערכת ולא בפרטייך, והמנהלים מסדרים אותה.'
	),
	array(
		0 => 'מדוע אני צריך להרשם בכלל?',
		1 => 'אינך חייב, זה תלוי במנהלי המערכת והרשאותיהם בפורום, יתכן ותהייה חייב להרשם בכדי לשלוח הודעות בפורום. בכל מקרה; הרישום יתן לך גישה לכלים אשר יעזרו וישפרו לך את חווית הגלישה בפורום דבר אשר לא תקבל כאורח, כלים כמו רישום ומעקב אחר נושאים, סמל אישי, חתימה, עריכת פרופיל משתמש ועוד. הרישום הינו לוקח דקות ספורות בלבד, כך שכדאי לך להרשם.'
	),
	array(
	...
	array(
		...
		1 => 'בעזרת לוח הבקרה למשתמש, תחת הכותרת “אפשרויות מערכת”,אתה תמצא אפשרות <em>הסתר את מצבי כמחובר</em>. הפעל אפשרות זו <samp>כן</samp>ורק מנהלי המערכת, מנהלי פורומים ואתה עצמך תיהיו אלה שיראו אותך מחובר. אתה תספר כמשתמש מוסתר.'
	),
	array(
		0 => 'איבדתי את הסיסמה שלי!',
		1 => 'אל תלחץ! כאשר אינך זוכר את הסיסמה, בקלות תוכל לאפסה. בקר בעמוד ההתחברות ולחץ <em>שחכתי סיסמה</em>. עקוב אחר ההוראות ותוכל להתחבר שוב בקרוב.'
	),
	array(
		0 => 'נרשמתי אבל אני לא מצליח להתחבר!',
		1 => 'ראשית, בדוק את שם המשתמש והסיסמה שהזנת. אם הם נכונים, אז כנראה ואת מהדברים הבאים קרה. אם מערכת ה COPPA פועלת במערכת ובהרשמה סימנת שאתה מתחת לגיל 13, תצטרך לעקוב אחר ההוראות שתקבל. בחלק ממערכות הפורומים דורשים את הפעלת החשבון, על ידי דואר אלקטרוני או מנהל המערכת; מידע זה מוצג במהלך ההרשמה. אם האישור להרשמה נשלח לדואר האלקטרוני, עקוב אחר ההוראות. אם לא קיבלת הודעה לדואר האלקטרוני, כנראה ונתת כתובת דואר אלקטרוני שגויה או שמערכת הדואר האלקטרוני העבירה את הודעת האישור בסינון הספאם. אם אתה בטוח שהפרטים שהזנת נכונים ודואר האלקטרוני אכן נכונה, צור קשר עם מנהל המערכת.'
	),
	array(
		0 => 'נרשמתי בעבר אבל אני לא מצליח להתחבר יותר?!',
		1 => 'קיימת אפשרות שמנהל ראשי כיבה או מחק את חשבונך מסיבה כלשהי. בנוסף, פורומים רבים מוחקים משתמשים אשר לא פירסמו הודעות זמן רב כדי לצמצם בגודל של בסיס הנתונים. אם זה קרה, נסה להרשם שוב ולהיות יותר פעיל בדיונים.'
	),
	array(
		0 => 'מהו COPPA?',
	...
	array(
		...
		1 => 'יכול להיות שמנהל המערכת חסם את כתובת ה IP שלך או אסר שימוש בשם משתמש זה. בנוסף מנהל המערכת יכול להפסיק את ההרשמה למערכת ובכך למנוע ממשתמשים חדשים להרשם. צור קשר עם מנהלי המערכת לעזרה.',
	),
	array(
	...
	array(
		...
		1 => 'יכול להיות שהזמן המוצג שונה מהזמנים באזורך. אם זאת הבעיה, בקר בלוח הבקרה למשתמש ושנה את הזמן על פי אזורך, לדוגמא לונדון, פאריס, ניו יורק, וכדומה. שים לב ששינוי אזור הזמן, כמו רוב ההגדרות, ניתן אך ורק למשתמשים רשומים. אם אינך רשום במערכת, זה הזמן הנכון להרשם.'
	),
	array(
	...
	array(
		...
		1 => 'מנהלי המערכת כנראה ולא העלו את השפה בהתקנת המערכת או שאף אחד לא תרגם את המערכת לשפה שלך. נסה לשאול את מנהלי המערכת אם ביכולתם להתקין את השפה. אם חבילת השפה אינה קיימת, תרגיש חופשי לנסות ולתרגם את השפה בעצמך. יותר מידע אודות תירגום שפה תוכל למצוא באתר הרישמי של PHPBB (ראה קישור בתחתית הפורום).'
	),
	array(
	...
	array(
		...
		1 => 'כדי לשלוח נושא חדש בפורום, לחץ על הכפתור הדרוש במסכי הפורום או הנושא. יתכן ותצטרך להרשם לפני שתוכל לשלוח הודעה. רשימת ההרשאות שלך בכל פורום זמינה בתחתית מסכי הפורום והנושא. לדוגמה: אתה יכול לשלוח נושאים חדשים, אתה יכול להצביע בסקרים, וכד\'.'
	),
	array(
	...
	array(
		0 => 'מדוע איני יכול להכנס לפורום?',
		1 => 'חלק מהפורומים מוגבלים לקבוצות משתמשים מסוימות. בכדי לצפות, לקרוא, לשלוח או לערוך אתה צריך גישות מסויימות, פנה למנהל המערכת בשביל לקבלם.'
	),
	array(
		0 => 'מדוע אני לא יכול להוסיף קבצים מצורפים?',
		1 => 'הרשאות צירוף קבצים נקבעות בכל פורום, לכל קבוצה, או לכל משתמש בפרט. המנהל הראשי של המערכת יכול לא לאפשר צירוף קבצים לפורום המסוים בו אתה שולח, או יתכן ורק קבוצות מסויימות יכולות לצרף קבצים. צור קשר עם המנהל הראשי של המערכת אם אינך בטוח מדוע אינך יכול לצרף קבצים.'
	),
	array(
		0 => 'מדוע קיבלתי אזהרה?',
		1 => 'כל מנהל ראשי של מערכת קובע את חוקי האתר שלו. אם עברת על חוק, יתכן וקיבלת אזהרה. שים לב כי זוהי החלטת המנהל הראשי של המערכת, וקבוצת phpBB לא יכולה לעשות דבר עם האזהרות באתר הנתון. צור קשר עם המנהל הראשי של המערכת אם אינך בטוח מדוע קיבלת אזהרה.'
	),
	array(
	...
	array(
		...
		1 => 'המנהל הראשי של המערכת יכול להחליט שההודעות בפורום בו אתה מנסה לכתוב נדרשות להבדק לפני הצגתן. יתכן גם שהמנהל הראשי הכניס אותך לקבוצה של משתמשים אשר ההודעות שלהם צריכות להבדקת לפני הצגתן. אנא צור קשר על המנהל הראשי של המערכת למידע נוסף.'
	),
	array(
	...
	array(
		0 => 'מה זה BBCode?',
		1 => 'BBCode הוא צורה מיוחדת של HTML, המציע שליטה נהדרת בעיצוב בחלקים מסויימים בהודעה. השימוש ב BBCode נקבע על-ידי המנהל הראשי, אבל ניתן גם לכבות אותו בכל הודעה בפרט מטופס השליחה. BBCode עצמו דומה במבנה ל HTML, אך התגים תחמים בסוגריים המרובעים [ ו ] במקום ב &lt; ו &gt;. למידע נוסף על BBCode ראה את המדריך אליו ניתן לגשת מעמוד השליחה.'
	),
	array(
	...
	array(
		...
		1 => 'כן, ניתן להציג תמונות בהודעות שלך. אם המנהל הראשי מאפשר צירוף קבצים, תוכל גם להעלות את התמונה למערכת. אחרת, אתה חייב לקשר לתמונה המאוחסנת בשרת רחוק הנגיש לכולם, למשל http://www.example.com/my-picture.gif. אינך יכול לקשר לתמונות המאוחסנות על המחשב האישי שלך (אלא אם כן הוא שרת הנגיש לכולם) ולא תמונות המאוחסנות מאחורי מנגנוני אימות, למשל תיבות הדואר של hotmail או yahoo, אתרים המוגנים בסיסמה, וכד\'. כדי להציג את התמונה בעזרת התג [img] של BBCode.'
	),
	array(
		0 => 'מה הן הכרזות גלובאליות?',
		1 => 'הכרזות גלובאליות מכילות מידע חשוב ואתה צריך לקרוא אותן בכל שעה אפשרית. הן יופיעו בראש של כל פורום ובלוח הבקרה למשתמש שלך. הרשאות הכרזה גלובאלית נקבעות על-ידי המנהל הראשי של המערכת.'
	),
	array(
		0 => 'מה הן הכרזות?',
		1 => 'הכרזות בדרך כלל מכילות מידע חשוב לפורום בו אתה כרגע קורא וצריך לקרוא אותן מתי שניתן. ההכרזות מופיעות בראש של כל עמוד בפורום בו הן נשלחו. כמו בהכרזות הגלובאליות, הרשאות הכרזה נקבעות על-ידי המנהל הראשי של המערכת.'
	),
	array(
		0 => 'מה הם נושאים דביקים?',
		1 => 'נושאים דביקים מופיעים בפורום מתחת להכרזות ורק בעמוד הראשון. הם בדרך כלל חשובים כך שאתה אמור לקרוא אותם בכל שעה נתונה. כמו בהכרזות ובהכרזות הגלובאליות, הרשאות נושא דביק נקבעות על-ידי המנהל הראשי של המערכת.'
	),
	array(
	...
	array(
		...
		1 => 'ראש קבוצת משתמשים נקבע בדרך כלל בעת יצירת הקבוצה על-ידי המנהל הראשי של המערכת. אם אתה מעונין ביצירת קבוצת משתמשים, אתה צריך ראשית ליצור קשר עם המנהל הראשי. נסה שליחת הודעה פרטית.',
	),
	array(
		0 => 'למה קבוצות משתמשים מסויימות מופיעות בצבעים שונים?',
	...
	array(
		...
		1 => 'אתה יכול לחסום משתמש משליחת הודעות פרטיות אליך על-ידי שימוש בכללי הודעות בלוח הבקרה למשתמש שלך. אם אתה מקבל הודעות פרטיות פוגעות ממשתמש מסויים, הודע למנהל הראשי. הוא יכול למנוע מהמשתמש משליחת הודעות פרטיות.'
	),
	array(
	...
	array(
		...
		1 => 'אתה יכול להוסיף משתמשים לרשימה שלך בשתי דרכים. בתוך כל פרופיל משתמש, ישנו קישור להוספת המשתמש לרשימת החברים או הנודניקים שלך. לחילופין, מלוח הבקרה למשתמש שלך, אתה יכול להוסיף ישירות משתמשים על-ידי הזנת שמות המשתמשים שלהם. אתה יכול גם להסיר משתמשים מהרשימה שלך בעזרת אותו עמוד.'
	),
	array(
		0 => '--',
		1 => 'חיפוש בפורומים'
	...
	array(
		0 => 'כיצד אני נרשם לנושא או לפורום מסויים?',
		1 => 'כדי להרשם לפורום מסויים, לחץ על הקישור “הרשם לקבלת עדכונים מפורום זה” לאחר הכניסה לפורום. כדי להרשם לנושא, הגב לנושא עם תיבת הסימון להרשמה מסומנת או לחץ על הקישור “הרשם לקבלת עדכונים מנושא זה” בנושא עצמו.'
	),
	array(
	...
	array(
		...
		1 => 'מנהל המערכת רשאי להוסיף ולהוריד אלו סוגי קבצים שברצונו לקבל או לא לקבל למערכת שלו. אם אינך בטוח שניתן להעאלות, צור קשר עם אחד ממנהלי המערכת למידע נרחב יותר.'
	),
	array(
	...
	array(
		0 => 'מי תיכנן וכתב את מערכת הפורומים?',
		1 => 'תוכנה זו (בצורה הלא ערוכה שלה ) נוצרה, ושוחררה וזכויותה הם של <a href="http://www.phpbb.com/">קבוצת phpBB</a>. המערכת נבנתה תחת רישיון חופשי וניתנת לעריכה חופשית ומלאה. ראה את הקישור לפרטים נוספים.'
	),
	array(
		0 => 'מדוע אפשרות X ניתנת?',
		1 => 'המערכת נכתבה וקיבלה רישיון על ידי קבוצת phpBB. אם אתה רואה שיש אפשרות שלא קיימת ותועיל מאוד, אנא בקר באתר הרישמי phpbb.com ותקבל את תגובות המפתחים לגבי העיניין. אנא אל תשלח בקשות עתידיות למערכת ב phpbb.com.אנא ראה את הפורום קודם וודא שלא שלחו כבר רעיון כמו שלך.'
	),
	array(
	...
	array(
		...
		1 => 'לא ולא! הרישיון הוא הינו האומנם רישיון חופשי אך אוסר על הסרת זכויות היוצרים והמתרגמים גם יחד ואין לשנותם. תנו כבוד ליוצרי המערכת ועורכייה.'
)
);
