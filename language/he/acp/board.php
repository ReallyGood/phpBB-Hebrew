...
$lang = array_merge($lang, array(
	...
	'CONTACT_US_ENABLE_EXPLAIN'		=> 'העמוד הזה מאפשר למשתמשים לשלוח מיילים למנהלים הראשיים של המערכת',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_CDN_EXPLAIN'				=> 'אם הגדרה זו פעילה חלק מהקבצים ישלחו משרתי צד שלישי חיצוניים במקום השרת שלך
	זה מפחית את תעבורת הרשת אשר נצרכת משרתך.
	,
	אבל עלול להוות בעיית פרטיות עבור מנהלי קהילות מסוימים.
	. בהתקנת PHPBB רגילה,
	זה כולל טעינת "JQUERY" וכן
	וכן הפונט "open sans" מרשת התוכן של גוגל.',

		'ALLOW_LIVE_SEARCHES'			=> 'אפשר חיפוש בזמן אמת',
	'ALLOW_LIVE_SEARCHES_EXPLAIN'	=> 'אם אפשרות זו פעילה, 
	המשתמשים יקבלו הצעות השלמה במהלך מילוי חלקים מהשדות בפורום.',
	...
	'READ_NOTIFICATION_EXPIRE_DAYS'	=> 'זמן תצוגה להתראות',
		'READ_NOTIFICATION_EXPIRE_DAYS_EXPLAIN' => 'מספר הימים שלאחרם ההתראה תמחק
	. כתוב 0 אם ברצונך שהתראות לא ימחקו.',
...
$lang = array_merge($lang, array(
	...
	'AUTH_PROVIDER_OAUTH_ERROR_ELEMENT_MISSING'	=> 'Both the key and secret of each enabled OAuth service provider must be provided
	. Only one was provided for an OAuth service provider.',
		'AUTH_PROVIDER_OAUTH_ERROR_ELEMENT_MISSING'	=> 'הן המפתח והסוד של
		כל שירות OAuth 
		חייב להיות מוזן
	. רק אחד סופק עבור שירות התוכן OAUTH.',
	'AUTH_PROVIDER_OAUTH_EXPLAIN'				=> 'על מנת לבצע אימות עם שרת חיצוני, כל ספק OAUTH דורש מפתח וסוד.
	זה אמור להיות מספק על ידי ספק ה-OAUTH כאשר אתה נרשם לאתרך איתם, ואמור להיות מוזן בדיוק כפי שניתן
	.<br />
כל שירות שאין לו את המפתח והסוד לא יוכל להיות בשימוש על ידי משתמשי הפורום.
	. כמו כן שים לב, שהמשתמשים עדיין יכולים להירשם ולהתחבר באמצעות תוסף ה-DB שלהם.',
...
$lang = array_merge($lang, array(
	...
	'MOD_REWRITE_ENABLE_EXPLAIN' => 'כאשר מופעל
	, כתובות URL אשר מכילות "app.php" ישונו כך ששם הקובץ ימחק .
	(i.e. app.php/foo will become /foo). 
	(למשל: app.php/foo יהיה /foo
	<strong>מודל mod_rewrite של שרת אפאצ\'י חייב לעבוד על מנת שההגדרה תעבוד;
	אם ההגדרה תופעל בלעדיו,
	קישורים באתרך עלולים לא לעבוד..</strong>',
	'MOD_REWRITE_DISABLED'		=> 'The <strong>mod_rewrite</strong> module on your Apache web server is disabled.
	Enable the module or contact your web hosting provider if you wish to enable this feature.',
		'MOD_REWRITE_DISABLED'		=> 'מודל ה-mod_rewrite בשרת האפצ\' שלך לא פעיל.
		הפעל אותו, או צור קשר עם בעל האחסון אם ברצונך להפעיל את אפשרות זו.',
	'MOD_REWRITE_INFORMATION_UNAVAILABLE' => 'אין באפשרותנו לקבוע האם השרת תומך בשינוי כתובת URL
	. אתה יכול להפעיל את ההגדרה, אך במידה ואפשרות זה לא נתמכת,
		נתיבים שיוספקו על ידי המערכת (וכן השימוש בלינקים) לא יעבוד.
	צור קשר עם בעל האחסון, אם אתה לא בטוח האם אתה יכול להשתמש באפשרות זו בביטחון.',
	...
	'USE_SYSTEM_CRON'			=> 
	'השתמש ב
	מריצה משימות מתוזמנות על ידי מריץ המשימות האוטומטי.',
	'USE_SYSTEM_CRON_EXPLAIN'	=> 'כאשר מכובה
	phpBB תארגן כך שמשימות מחזוריות יורצו אוטמטית
	. כאשר דלוקה
	, phpBB לא תארגן שום משימה שתרוץ בצורה עצמאית
	מנהל המערכת יהיה חייב להגדיר ש-
	<code>bin/phpbbcli.php cron:run</code>
	יופעל על ידי המערכת בהפרשי זמנים סבירים באמצעות מערכת cron
	(לדוגמה כל 5 דקות).',
));
...
$lang = array_merge($lang, array(
	...
	'ALLOW_PASSWORD_RESET_EXPLAIN'	=> 'קובע האם משתמשים יוכלו להשתמש באפשרות
"שכחתי את סיסמתי" הנמצאת בקישור בעמוד ההתחברות, על מנת לקבל גישה מחודשת לחשבונם. אם אתה משתמש בשיטת אימות חיצונית, יתכן שתרצה לבטל את האפשרות הזאת.
	.',
